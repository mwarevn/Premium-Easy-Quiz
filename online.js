...
`);btnContribute.on("click",["target"],e=>{e.set("disabled",!0);const t=DOM.findAll("textarea[name=ques]").map((e,t)=>[e,DOM.findAll("textarea[name=ans]")[t]]);var a=t.map(([e,t])=>({ques:capitalize(e.value().trim()),ans:capitalize(t.value().trim())})).filter(e=>e.ques&&e.ans);console.debug(a),a.length?addQuiz(a):(alert("Bạn chưa nhập câu nào!"),e.set("disabled",!1))});const btnAddItem=DOM.create(`
