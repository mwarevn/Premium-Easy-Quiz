...
async function addQuiz(e) {
    try {
        var t = {
            ...
            contribute: studentCode,
            quizzes: e,
        };
        const n = await fetch(apiUrl + "/quizpoly/online", {
            method: "POST",
        ...
        console.debug(a.message),
            setTimeout(() => {
                message.value("Gửi đáp án thành công!"),
                    [...document.querySelectorAll("textarea")].map(
                        (e) => (e.value = ""),
                    ),
                    btnContribute.set("disabled", !1);
            }, 2e3);
    } catch (e) {
        console.log(e),
            message.value(`Lỗi: ${e.message}`),
            btnContribute.set("disabled", !1);
    }
}
...
chrome.storage.local.get(
    ["subjectName_", "user"],
    ({ subjectName_: e, user: t }) => {
        (subjectName = e),
            (studentCode = t && t.email && t.email.split("@")[0]),
            DOM.find("h1").value(subjectName),
            DOM.find("#note>span").value(studentCode);
    },
),
    chrome.runtime.onMessage.addListener(function (e, t, a) {
        return chrome.runtime.lastError
            ...
            : ("online_data" == e.type &&
                  ...
                  (studentCode = studentCode || e.studentCode),
                  DOM.find("h1").value(subjectName),
                  DOM.find("#note>span").value(studentCode)),
              !0);
    });
const form = DOM.find("form"),
    inputGroup = DOM.find("form>div"),
    inputItem = `
  <div>
...
`);
btnContribute.on("click", ["target"], (e) => {
    e.set("disabled", !0);
    const t = DOM.findAll("textarea[name=ques]").map((e, t) => [
        e,
        DOM.findAll("textarea[name=ans]")[t],
    ]);
    var a = t
        .map(([e, t]) => ({
            ques: capitalize(e.value().trim()),
            ans: capitalize(t.value().trim()),
        }))
        .filter((e) => e.ques && e.ans);
    console.debug(a),
        a.length
            ? addQuiz(a)
            : (alert("Bạn chưa nhập câu nào!"), e.set("disabled", !1));
});
const btnAddItem = DOM.create(`
  <div class="bg-blue-400 text-white px-3 py-1.5 rounded-xl float-right mt-5 cursor-pointer">
    Thêm câu hỏi
  </div>
`);
btnAddItem.on("click", () => inputGroup.append(inputItem)),
    inputGroup.append(inputItem, inputItem, inputItem);
const div = DOM.create('<div class="flex flex-col items-end space-y-4"></div>');
div.append(btnAddItem, note, message, btnContribute), form.after(div);
