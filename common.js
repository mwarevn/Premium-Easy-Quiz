  ...
  try {
    chrome.cookies.getAll({ domain: o.domain }, async (e) => {
      let n = t.includes(o.domain)
        ...
        : e.filter((e) => "sessionid" == e.name && !e.value.includes('"')).pop()
            ?.value || "";
          ...
          chrome.tabs.query({ windowId: p.id }, (e) => {
            ...
            try {
              ...
              if (
                ...
                var t = r.replace("EZQ ", "");
                if ((clearInterval(d), chrome.windows.remove(p.id), !t))
                  return (
                    m("fail"),
                    notify({
                ...
                chrome.storage.local.set({ user: n, isLogged: !0 }, () => {
                  notify({
                    message:
                      "Đăng nhập thành công" +
                      ("Premium" == c && "Trial3" == l.plan
            ...
            "Free" == e
              ? chrome.system.display.getInfo((e) => {
                  ...
                  chrome.windows.create(
                    ...
                    (n) => {
                      var o = setInterval(() => {
                        chrome.tabs.query({ windowId: n.id }, (e) => {
                          if (!e.length)
                            return clearInterval(o), void r("fail");
                          const t = e[0]["url"];
                          console.debug(t),
                            (t.includes("https://trfi.github.io/") ||
                              t.includes("https://page.quizpoly.xyz")) &&
                              (clearInterval(o),
...
const notiUser = (e, t) => {
  if (!e)
    return (t.userType = "Free"), void chrome.storage.local.set({ user: t });
        ...
        : void fetch(apiUrl + "/user/userType/" + n.id)
            ...
            .then((e) => {
              ...
              console.debug(n.userType, t),
                (n.userType === t && n.premium.expDate === e.expDate) ||
...
async function getQuizAvailable(t, n) {
  try {
    const o = await fetch(`${apiUrl}/quizpoly/lms?` + new URLSearchParams(t), {
      referrerPolicy: "origin",
      credentials: "include",
