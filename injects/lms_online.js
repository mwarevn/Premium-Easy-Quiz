...
async function main() {
	if (subject) {
		...
		if (e && e.length) {
			if (5 <= e.length) if (!(await u())) return chrome.runtime.sendMessage({ type: "close_quiz_popup" });
...
function writeHTML(e) {
    ...
    `);
	setTimeout(() => chrome.runtime.sendMessage({ type: "quiz_data", html: t, online: !0 }), 1e3);
}
